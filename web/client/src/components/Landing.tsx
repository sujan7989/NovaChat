...
function frac(s: number) { return s / 233280; }

function DeepSpaceBg() {
  // Dense star field — mix of white, pink, purple tones like the reference
  const stars = Array.from({ length: 340 }, (_, i) => {
    ...
    // Heavily pink/purple weighted like the reference image
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {meteors.map((m, i) => (
        <div key={`m${i}`} className="absolute" style={{
      ...
      {/* Large purple nebula — top left, like reference */}
      <div className="absolute" style={{
        top: "-5%", left: "-5%", width: 700, height: 700,
        background: "radial-gradient(circle,rgba(130,40,200,0.4) 0%,rgba(90,20,160,0.15) 30%,rgba(60,10,120,0.05) 60%,transparent 100%)",
        filter: "blur(60px)", animation: "auroraShift 24s ease-in-out infinite",
      }} />
      {/* Magenta nebula — right side */}
      <div className="absolute" style={{
        top: "10%", right: "-8%", width: 600, height: 600,
        background: "radial-gradient(circle,rgba(200,30,140,0.35) 0%,rgba(160,20,120,0.12) 35%,rgba(100,10,80,0.03) 65%,transparent 100%)",
        filter: "blur(65px)", animation: "auroraShift2 20s ease-in-out infinite",
      }} />
      {/* Bottom left purple */}
      <div className="absolute" style={{
        bottom: "0%", left: "5%", width: 500, height: 500,
        background: "radial-gradient(circle,rgba(110,30,190,0.3) 0%,rgba(80,10,150,0.1) 40%,rgba(40,5,100,0.02) 70%,transparent 100%)",
        filter: "blur(55px)", animation: "auroraShift 30s ease-in-out 8s infinite",
      }} />
      {/* Bottom right pink */}
      <div className="absolute" style={{
        bottom: "5%", right: "5%", width: 450, height: 450,
        background: "radial-gradient(circle,rgba(220,50,150,0.25) 0%,rgba(180,30,120,0.08) 45%,rgba(120,15,80,0.02) 75%,transparent 100%)",
        filter: "blur(55px)", animation: "auroraShift2 26s ease-in-out 4s infinite",
      }} />
      {/* Center floor glow — the signature horizontal glow from the reference */}
      <div className="absolute" style={{
        bottom: "8%", left: "50%", transform: "translateX(-50%)",
        width: "80%", height: 180,
        background: "radial-gradient(ellipse,rgba(180,50,255,0.4) 0%,rgba(240,60,180,0.2) 35%,rgba(200,40,140,0.05) 60%,transparent 100%)",
        filter: "blur(40px)",
      }} />
      {/* Subtle center core glow behind ghost */}
      <div className="absolute" style={{
        top: "20%", left: "50%", transform: "translateX(-50%)",
        width: 500, height: 500,
        background: "radial-gradient(circle,rgba(140,60,220,0.15) 0%,rgba(100,30,180,0.05) 40%,rgba(60,15,120,0.01) 70%,transparent 100%)",
        filter: "blur(45px)",
      }} />
    </div>
  );
...
export default function Landing({ onStart }: Props) {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(iv); socket.off("online_count"); socket.disconnect(); };
  }, []);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (i >= steps) { countRef.current = target; clearInterval(iv); }
      setDisplayOnline(countRef.current);
    }, 35);
    return () => clearInterval(iv);
  }, [stats.online]);

  const handleStart = () => { setLeaving(true); setTimeout(onStart, 380); };

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === "Enter" && !leaving) handleStart(); };
  ...
  return (
    <div className="h-full w-full flex items-center justify-center relative overflow-hidden"
      ...
      <div className={`relative z-10 flex flex-col items-center text-center px-6 max-w-xl w-full transition-all duration-500 ease-in-out
        ${visible && !leaving ? "opacity-100 translate-y-0" : leaving ? "opacity-0 -translate-y-8 scale-95" : "opacity-0 translate-y-10"}`}>

        {/* Online badge — pill style matching reference */}
        <div className="flex items-center gap-2.5 px-5 py-2 rounded-full mb-7"
          style={{
        ...
        {/* Title — big, bold, purple→pink gradient like reference */}
        ...
        <p className="text-base leading-relaxed mb-6" style={{ color: "rgba(255,255,255,0.55)", maxWidth: 380 }}>
          Modern anonymous chat with strangers.<br />No account. No name. Just connections.
        </p>

        {/* Feature pills */}
        <div className="flex flex-wrap justify-center gap-2 mb-5">
          {features.map(f => (
            <div key={f.label} className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold"
              style={{
        ...
        {/* Online count line — like reference's "🔥 +1,200 people online" */}
        ...
        {/* CTA button — glowing border pill like reference */}
        <div className="relative w-full max-w-sm mb-5">
          {/* Outer glow */}
          <div className="absolute inset-0 rounded-full" style={{
            background: "linear-gradient(135deg,#7c3aed,#db2777)",
            filter: "blur(20px)", opacity: 0.75, transform: "scaleY(0.55) translateY(14px)",
          }} />
          {/* Border glow ring */}
          <div className="absolute inset-0 rounded-full" style={{
        ...
        {/* Stats row — compact under button like reference */}
